/*
 * mirisdr, turns your Mirics based DVB dongle into a SDR receiver
...
/*
 * miri_power: general purpose FFT integrator
...
{
	fprintf(stderr,
		"miri_power, a simple FFT logger for RTL2832 based DVB-T receivers\n\n"
		"Use:\tmiri_power -f freq_range [-options] [filename]\n"
		...
		"Examples:\n"
		"\tmiri_power -f 88M:108M:125k fm_stations.csv\n"
		"\t (creates 160 bins across the FM band,\n"
		"\t  individual stations should be visible)\n"
		"\tmiri_power -f 100M:1G:1M -i 5m -1 survey.csv\n"
		"\t (a five minute low res scan of nearly everything)\n"
		"\tmiri_power -f ... -i 15m -1 log.csv\n"
		"\t (integrate for 15 minutes and exit afterwards)\n"
		"\tmiri_power -f ... -e 1h | gzip > log.csv.gz\n"
...
void retune(mirisdr_dev_t *d, int freq)
{
	uint8_t dump[BUFFER_DUMP];
	int n_read;
	mirisdr_set_center_freq(d, (uint32_t)freq);
	/* wait for settling and flush buffer */
	usleep(5000);
	mirisdr_read_sync(d, &dump, BUFFER_DUMP, &n_read);
...
void scanner(void)
{
	...
	for (i=0; i<tune_count; i++) {
		if (do_exit >= 2)
			{return;}
		ts = &tunes[i];
		f = (int)mirisdr_get_center_freq(dev);
		if (f != ts->freq) {
			retune(dev, ts->freq);}
		mirisdr_read_sync(dev, ts->buf8, buf_len, &n_read);
		if (n_read != buf_len) {
...
int main(int argc, char **argv)
{
	...
	mirisdr_set_bias(dev, enable_biastee);
	if (enable_biastee)
	...
	/* actually do stuff */
	mirisdr_set_sample_rate(dev, (uint32_t)tunes[0].rate);
